...
pub(super) fn resolve_impl_headers(
    ...
    let def_map = def_map.read_txn(unexpected_package_loader());
    let semantic_ir = db.read_txn(unexpected_package_loader());
...
pub(super) fn unexpected_package_loader<T: 'static>() -> PackageLoader<'static, T> {
