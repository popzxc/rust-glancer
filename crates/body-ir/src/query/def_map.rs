...
pub(crate) trait DefMapQuery {
    fn resolve_path(&self, from: ModuleRef, path: &Path) -> ResolvePathResult;

    fn resolve_path_in_type_namespace(&self, from: ModuleRef, path: &Path) -> ResolvePathResult;
}

impl DefMapQuery for DefMapDb {
    fn resolve_path(&self, from: ModuleRef, path: &Path) -> ResolvePathResult {
        DefMapDb::resolve_path(self, from, path)
    }

    fn resolve_path_in_type_namespace(&self, from: ModuleRef, path: &Path) -> ResolvePathResult {
        DefMapDb::resolve_path_in_type_namespace(self, from, path)
    }
}
